...
1. **Forecasting/matching/allocation (classical ML).** Demand and supply state → choose inventory/dispatch/offer → completed transaction with lower wait/markdown/cancellation → contribution and working-capital gain. Uber's named Michelangelo platform supports deployed ML workflows (E2) for ETA, matching and fraud; its 2024 engineering post reports 400 active ML projects, more than 5,000 production models, 20,000 monthly training jobs and peak serving of 10m predictions per second, while documenting its extension to LLMOps. These are platform-scale activity claims, not incremental net cash. Consolidated growth does not isolate a new model's causal effect.
